...
export interface PaperConfig {
  facePaper: number; // 面纸克重
  innerPaper: number; // 里纸克重
  mediumPaper: number; // 芯纸克重
}
...
export interface CostBreakdown {
  paperCost: number; // 纸料成本
  lossCost: number; // 损耗成本
  craftCost: number; // 工艺成本
  profit: number; // 利润
  tax: number; // 税费
  total: number; // 总计
}
...
export interface SystemConfig {
  paperPrice: number; // 当前纸板单价
  defaultLossRate: number; // 默认损耗率
